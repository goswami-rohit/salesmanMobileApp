  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        console.error("Failed to fetch regions/areas:", err);
      }
    })();
  }, []);
  ...
  const submit = async (values: SalesOrderFormValues) => {
    try {
      ...
      const payload = {
        ...
        userId: user?.id ?? null, // if schema needs it
      };
            ...
            <View className="flex-1 mb-4">
              <View className="p-3 bg-slate-800 rounded-lg border border-slate-600">
                <RNPickerSelect
                  ...
                  value={value}
                  items={areas.map(a => ({ label: a, value: a }))}
                  placeholder={{ label: "Area *", value: null }}
