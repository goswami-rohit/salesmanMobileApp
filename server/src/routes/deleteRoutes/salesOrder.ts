//  server/src/routes/dataFetchingRoutes/salesOrders.ts 
// Sales Orders DELETE endpoints using createAutoCRUD pattern
...
function createAutoCRUD(app: Express, config: {
  ...
  // DELETE BY Salesman ID
  app.delete(`/api/${endpoint}/salesman/:salesmanId`, async (req: Request, res: Response) => {
    try {
      ...
      const recordsToDelete = await db.select().from(table).where(eq(table.salesmanId, parseInt(salesmanId)));
      ...
      // Delete all records for this salesman
      await db.delete(table).where(eq(table.salesmanId, parseInt(salesmanId)));
