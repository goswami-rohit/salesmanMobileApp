...
function createAutoCRUD(app: Express, config: {
  ...
  // DELETE BY Leave Type
  app.delete(`/api/${endpoint}/leave-type/:leaveType`, async (req: Request, res: Response) => {
    try {
      ...
      // Get count of records to be deleted
      const recordsToDelete = await db.select().from(table).where(eq(table.leaveType, leaveType));
      ...
      // Delete all records with this leave type
      await db.delete(table).where(eq(table.leaveType, leaveType));
...
export default function setupSalesmanLeaveApplicationsDeleteRoutes(app: Express) {
  // Salesman Leave Applications DELETE endpoints
  createAutoCRUD(app, {
