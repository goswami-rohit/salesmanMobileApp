//  server/src/routes/dataFetchingRoutes/salesReport.ts 
// Sales Report DELETE endpoints using createAutoCRUD pattern
...
function createAutoCRUD(app: Express, config: {
  ...
  // DELETE BY Sales Person ID
  app.delete(`/api/${endpoint}/salesperson/:salesPersonId`, async (req: Request, res: Response) => {
    try {
      ...
      const recordsToDelete = await db.select().from(table).where(eq(table.salesPersonId, parseInt(salesPersonId)));
      ...
      // Delete all records for this sales person
      await db.delete(table).where(eq(table.salesPersonId, parseInt(salesPersonId)));
  ...
  app.delete(`/api/${endpoint}/dealer/:dealerId`, async (req: Request, res: Response) => {
    try {
      ...
      // Get count of records to be deleted
      const recordsToDelete = await db.select().from(table).where(eq(table.dealerId, dealerId));
...
export default function setupSalesReportDeleteRoutes(app: Express) {
  // Sales Report DELETE endpoints
  createAutoCRUD(app, {
