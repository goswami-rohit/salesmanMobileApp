...
function createAutoCRUD(app: Express, config: {
  ...
  app.get(`/api/${endpoint}`, async (req: Request, res: Response) => {
    try {
      ...
      // Date range filtering using creationDate
...
export default function setupDdpRoutes(app: Express) {
  // DDP - date field for filtering
  createAutoCRUD(app, {
    endpoint: 'ddp',
    table: ddp,
    ...
    autoFields: {
      creationDate: () => new Date().toISOString().split('T')[0] // date type
    }
  });
