...
function createAutoCRUD(app: Express, config: {
  ...
  app.get(`/api/${endpoint}`, async (req: Request, res: Response) => {
    try {
      ...
      // Date range filtering using planDate
      ...
      if (createdById) {
        whereCondition = whereCondition 
          ? and(whereCondition, eq(table.createdById, parseInt(createdById as string)))
          : eq(table.createdById, parseInt(createdById as string));
      }
      ...
      // Order by planDate if available, otherwise createdAt
  ...
  // GET BY User ID - plans assigned to user
  ...
  // GET BY Created By User ID - plans created by user
  app.get(`/api/${endpoint}/created-by/:createdById`, async (req: Request, res: Response) => {
    try {
      ...
      let whereCondition = eq(table.createdById, parseInt(createdById));
  ...
  app.get(`/api/${endpoint}/status/:status`, async (req: Request, res: Response) => {
    try {
      ...
      if (createdById) {
        whereCondition = and(whereCondition, eq(table.createdById, parseInt(createdById as string)));
      }
...
export default function setupPJPRoutes(app: Express) {
  ...
  createAutoCRUD(app, {
    ...
    dateField: 'planDate',
    autoFields: {
      planDate: () => new Date().toISOString().split('T')[0], // date type
