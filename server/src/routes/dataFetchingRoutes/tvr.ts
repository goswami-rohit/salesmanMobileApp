...
function createAutoCRUD(app: Express, config: {
  ...
  app.get(`/api/${endpoint}`, async (req: Request, res: Response) => {
    try {
      ...
      if (serviceType) {
        whereCondition = whereCondition 
          ? and(whereCondition, eq(table.serviceType, serviceType as string))
          : eq(table.serviceType, serviceType as string);
      }
  ...
  app.get(`/api/${endpoint}/user/:userId`, async (req: Request, res: Response) => {
    try {
      ...
      if (serviceType) {
        whereCondition = and(whereCondition, eq(table.serviceType, serviceType as string));
      }
...
export default function setupTechnicalVisitReportsRoutes(app: Express) {
  // Technical Visit Reports - date field for filtering
  createAutoCRUD(app, {
